...
<form class="form-horizontal" action="<%=formUrl%>" id="myform" method="post" enctype="multipart/form-data">
	<fieldset>
			...
			<div class="col-md-5">
				<select id="doctype.dtid" name="doctype.dtid" class="form-control">
					<c:forEach items="${allDoctypes}" var="dtype">
						<option value="${dtype.dtid}">${dtype.title}</option>
					</c:forEach>
				</select>
			</div>
			<!-- 定义表单错误提示显示元素 -->
		...
		<div class="form-group" id="document.contentDiv">
			...
			<div class="col-md-5">
				<!-- 定义表单输入组件 -->
				<textarea id="document.content" name="document.content" class="form-control" placeholder="请输入文档的描述信息" rows="10"></textarea>
			</div>
			<!-- 定义表单错误提示显示元素 -->
