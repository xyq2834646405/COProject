$(function() {
	...
	$("#myform").validate({
		...
		submitHandler : function(form) {
			...
			$.post("pages/jsp/admin/document/DoctypeActionAdmin!insert.action",{"doctype.title":title},function (data) {
				if(data.flag == true){
					var dtid = data.dtid;
					addRow(dtid,title) ;
				}
				$("#docTypeInfo").modal("hide") ;
				operateAlert(data.flag,"文档类型信息增加成功！","文档类型信息增加失败！") ;
			},"json");
		},
...
function bindUpdateEvent(dtid) {
	$("#updateBtn-" + dtid).on("click",function() {
		...
		// 编写Ajax异步更新操作
		$.post("pages/jsp/admin/document/DoctypeActionAdmin!update.action",{"doctype.dtid":dtid,"doctype.title":title},function (data) {
			operateAlert(data=="true","文档类型信息修改成功！","文档类型信息修改失败！") ;
		},"text");
