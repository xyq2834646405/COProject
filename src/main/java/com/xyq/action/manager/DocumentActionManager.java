...
import com.xyq.service.manager.IDocumentServiceManager;
...
@ParentPackage("root")
@Results({
    @Result(name = "document.list",location = "/pages/jsp/manager/document/manager_document_list.jsp"),
    @Result(name = "document.list.admin",location = "/pages/jsp/manager/document/manager_document_admin.jsp"),
    @Result(name = "document.insert",location = "/pages/jsp/manager/document/manager_document_insert.jsp"),
    @Result(name = "document.update",location = "/pages/jsp/manager/document/manager_document_update.jsp"),
    @Result(name = "insertVF",location = "/pages/jsp/manager/document/manager_document_insert.jsp"),
    @Result(name = "updateVF",location = "/pages/jsp/manager/document/manager_document_update.jsp")
})
@Namespace("/pages/jsp/manager/document")
...
public class DocumentActionManager extends AbstractAction {
    ...
    @Autowired
    private IDocumentServiceManager documentServiceManager;

    public String insertPre(){
        try {
            Map<String, Object> map = documentServiceManager.insertPre();
    ...
    public String insert(){
        ...
        document.setUser(manager);
        if (file.length()>0){//有文件上传
            document.setFile(createSingleFileName(fileContentType));
        }
        try {
            if (documentServiceManager.insert(document)){
                String filePath = getApplication().getRealPath("/upload/document/")+document.getFile();
                if(file.length()>0){
                    saveSingleFile(filePath,file);
                }
                setMsgAndUrl("insert.success.msg","manager.document.insert.action");
            }else{
                setMsgAndUrl("insert.failure.msg","manager.document.insert.action");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String updatePre(){
        try {
            Map<String, Object> map = documentServiceManager.updatePre(document.getDid());
            getRequest().setAttribute("allDoctypes",map.get("allDoctypes"));
    ...
    public String update(){
        ...
        User manager = (User) getSession().getAttribute("manager");
        document.setUser(manager);//设置当前用户的信息
        document.setDoctype(doctype);
        try {
            if (documentServiceManager.update(document)){
                String filePath = getApplication().getRealPath("/upload/document/")+document.getFile();
                if(file.length()>0){
                    saveSingleFile(filePath,file);
                }
                setMsgAndUrl("update.success.msg","manager.document.list.admin.action");
            }else{
                setMsgAndUrl("update.failure.msg","manager.document.list.admin.action");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void delete(){
        ...
        if(ids!=null){
            ...
            try {
                if (documentServiceManager.delete(did,manager.getUserid())){
    ...
    public String list(){
        try {
            Map<String, Object> map = documentServiceManager.list(getCp(), getLs(), getCol(), getKw());
            handleSplit(map.get("documentCount"),"manager.document.split.url",null,null);
    ...
    public String admin(){
        try {
            User manager = (User) getSession().getAttribute("manager");
            Map<String, Object> map = documentServiceManager.listByUser(manager.getUserid(),getCp(), getLs(), getCol(), getKw());
            handleSplit(map.get("documentCount"),"manager.document.split.url",null,null);
            getRequest().setAttribute("allDocuments",map.get("allDocuments"));
            List<Doctype> dtlist = (List<Doctype>) map.get("allDoctypes");
            Map<Integer,String> typeMap = new HashMap<Integer, String>();
